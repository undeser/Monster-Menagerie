...
contract("Staking", function(accounts) {
    ...
    /**
     * TEST CASES MIGHT NOT PASS DUE TO ROUNDING ERRORS FROM BIG NUMBER
     */
    ...
    it("Should update rewards for accounts[1] after depositing", async () => {
        ...
        const pendingRewards1 = new BigNumber(await Stakingcontract.pendingReward(accounts[1])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const oneDayReward = new BigNumber(2739726027397209600).dividedBy(dividingFactor).decimalPlaces(0, 1);
        // console.log(pendingRewards1);
        // console.log(oneDayReward);
    ...
    it("Should claim rewards for accounts[1]", async () => {
        ...
        const reward = finalBalance.minus(initialBalance).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const oneDayReward = new BigNumber(2739726027397209600).dividedBy(dividingFactor).decimalPlaces(0, 1);

        // console.log(reward);
        // console.log(oneDayReward);

        assert(reward.isEqualTo(oneDayReward),"accounts[1] did not claim the correct rewards");
    ...
    it("The reward should split evenly after account[2] deposits", async () => {
        ...
        const pendingRewards2 = new BigNumber(await Stakingcontract.pendingReward(accounts[2])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        // Account 1 still have his balance staked from previously
        const pendingRewards1 = new BigNumber(await Stakingcontract.pendingReward(accounts[1])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const rewardSplit = new BigNumber(1369863013698604800).dividedBy(dividingFactor).decimalPlaces(0, 1);
        // console.log(pendingRewards1);
    ...
    it("Withdrawing", async () => {
        ...
        const reward = finalBalance.minus(initialBalance).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const halfReward = new BigNumber(1369863013698604800).dividedBy(dividingFactor).decimalPlaces(0, 1);
        // console.log(reward);
        // console.log(halfReward);

        assert(reward.isEqualTo(halfReward), "Withdrawal of reward is not the correct amount");
    ...
    it("Reward should be split proportionally after account[3] depositing", async() => {
        ...
        const account1Share = oneDayReward.times(4).div(7).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const account2Share = new BigNumber(oneDayReward.times(2).div(7)).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const account3Share = new BigNumber(oneDayReward.times(8).div(7)).dividedBy(dividingFactor).decimalPlaces(0, 1);

        const pendingRewards1 = new BigNumber(await Stakingcontract.pendingReward(accounts[1])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const pendingRewards2 = new BigNumber(await Stakingcontract.pendingReward(accounts[2])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        const pendingRewards3 = new BigNumber(await Stakingcontract.pendingReward(accounts[3])).dividedBy(dividingFactor).decimalPlaces(0, 1);
        // console.log(account1Share);
        // console.log(pendingRewards1);
        // console.log(account2Share);
        // console.log(pendingRewards2);
        // console.log(account3Share);
